...
class WorkflowSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if (!$taguigBranch) {
            ...
            $taguigWorkflow = Workflow::firstOrCreate(
                ['branch_id' => $taguigBranch->id, 'workflow_name' => 'Mang Inasal Taguig Hiring Process'],
                ['is_active' => true]
            );

            if ($taguigWorkflow->wasRecentlyCreated) {
                foreach ([
        ...
        if (!$velasquezBranch) {
            ...
            $velasquezWorkflow = Workflow::firstOrCreate(
                ['branch_id' => $velasquezBranch->id, 'workflow_name' => 'Mang Inasal Velasquez Hiring Process'],
                ['is_active' => true]
            );

            if ($velasquezWorkflow->wasRecentlyCreated) {
                foreach ([
        ...
        if (!$jaicomBranch) {
            ...
            $jaicomWorkflow = Workflow::firstOrCreate(
                ['branch_id' => $jaicomBranch->id, 'workflow_name' => 'Jaicom Ortigas Center Hiring Process'],
                ['is_active' => true]
            );

            if ($jaicomWorkflow->wasRecentlyCreated) {
                foreach ([
